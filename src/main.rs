mod utils;
use utils::{io::fuzzy_search,args::CliArgs};
...
fn main() {
   ...
   let mut tmp_current_pathbuf=PathBuf::new(); 
    ...
    let mut results:Vec<String>=Vec::<String>::new();
     println!("[+] Starting ");
    fuzzy_search(&file_name.as_str(), &args.is_directory.unwrap_or(false),&mut path, &mut None, &mut args.count.unwrap_or(1), &args.verbose.unwrap_or(false), &args.backward_search.unwrap_or(false),&mut results );
    println!("[+] Results : ");
    for i in results{
        println!("{}",i);
    }
